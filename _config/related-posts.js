...
export default function relatedPostsPlugin(eleventyConfig) {
	...
	eleventyConfig.addFilter("relatedPosts", function (currentPost, allPosts, options = {}) {
		const {
			maxResults = 3,
			sameAuthorWeight = 2,
			tagMatchWeight = 1,
		...
		const scoredPosts = candidatePosts.map((post) => {
			...
			const meaningfulCurrentTags = new Set([...currentTags].filter(tag => !excludeTags.has(tag)));
			const meaningfulPostTags = new Set([...postTags].filter(tag => !excludeTags.has(tag)));
			...
			score += sharedTags.length * tagMatchWeight;
			...
			const recencyScore = Math.max(0, recencyBonus * (365 - daysDiff) / 365);
			score += recencyScore;
			...
			return {
				...
				score,
				sharedTags,
				sameAuthor: post.data.author === currentPost.data.author
			};
		});
	...
	eleventyConfig.addFilter("postsBySameAuthor", function (currentPost, allPosts, limit = 3) {
		if (!currentPost || !allPosts) return [];

		return allPosts
			.filter((post) => {
				return post.data.author === currentPost.data.author &&
	...
	eleventyConfig.addFilter("postsWithSimilarTags", function (currentPost, allPosts, limit = 5) {
		if (!currentPost || !allPosts) return [];
		...
		// Get meaningful tags
		const meaningfulTags = new Set([...currentTags].filter(tag => !excludeTags.has(tag)));
		...
		const taggedPosts = allPosts
			.filter((post) => {
				...
				const postTags = new Set(post.data.tags || []);
				const postMeaningfulTags = new Set([...postTags].filter(tag => !excludeTags.has(tag)));
			...
			.map((post) => {
				const postTags = new Set(post.data.tags || []);
				const postMeaningfulTags = new Set([...postTags].filter(tag => !excludeTags.has(tag)));
				const sharedTags = [...meaningfulTags].filter(tag => postMeaningfulTags.has(tag));
	...
	eleventyConfig.addFilter("recentPosts", function (allPosts, excludePost = null, limit = 5) {
		if (!allPosts) return [];
	...
	eleventyConfig.addFilter("tagRelevance", function (currentPost, targetPost) {
		...
		const meaningfulCurrentTags = new Set([...currentTags].filter(tag => !excludeTags.has(tag)));
		const meaningfulTargetTags = new Set([...targetTags].filter(tag => !excludeTags.has(tag)));

		const sharedTags = [...meaningfulCurrentTags].filter(tag => meaningfulTargetTags.has(tag));
	...
	eleventyConfig.addShortcode("relatedPostsSection", function (currentPost, allPosts) {
		const relatedPosts = this.ctx.collections.posts
			? this.getFilteredByGlob(this.ctx.collections.posts, "relatedPosts", currentPost, { maxResults: 3 })
			: [];
	...
	eleventyConfig.addFilter("trendingTags", function (allPosts, daysBack = 30, limit = 10) {
		if (!allPosts) return [];
	...
	eleventyConfig.addFilter("authorActivity", function (allPosts, daysBack = 30) {
		if (!allPosts) return [];
...
export function calculatePostSimilarity(post1, post2, options = {}) {
	...
	const meaningfulTags1 = new Set([...tags1].filter(tag => !excludeTags.has(tag)));
	const meaningfulTags2 = new Set([...tags2].filter(tag => !excludeTags.has(tag)));

	const sharedTags = [...meaningfulTags1].filter(tag => meaningfulTags2.has(tag));
	const totalUniqueTags = new Set([...meaningfulTags1, ...meaningfulTags2]).size;
